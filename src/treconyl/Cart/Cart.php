...
use Treconyl\Cart\Exceptions\InvalidConditionException;
use Treconyl\Cart\Exceptions\InvalidItemException;
use Treconyl\Cart\Helpers\Helpers;
use Treconyl\Cart\Validators\CartItemValidator;
use Treconyl\Cart\Exceptions\UnknownModelException;

/**
 * Class Cart
 * @package Treconyl\Cart
 */
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = "\\Treconyl\\Cart\\CartCondition";

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of Treconyl\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) throw new InvalidConditionException('Argument 1 must be an instance of \'Treconyl\Cart\CartCondition\'');
